...
import com.jmj.common.UtilAll;
import com.jmj.common.util.store.config.FlushDiskType;
...
public class MappedFile extends ReferenceResource {
    ...
    private void init(final String fileName, final int fileSize) throws IOException {
        ...
        try {
            ...
            log.error("create file channel " + this.fileName + " Failed. ");
            throw e;
        } catch (IOException e) {
            log.error("map file " + this.fileName + " Failed. ");
            throw e;
        } finally {
        ...
        for (int i = 0, j = 0; i < this.fileSize; i += MappedFile.OS_PAGE_SIZE, j++) {
            ...
            if (type == FlushDiskType.SYNC_FLUSH) {
                if ((i / OS_PAGE_SIZE) - (flush / OS_PAGE_SIZE) >= pages) {
                    flush = i;
                    mappedByteBuffer.force();   //强制操作系统将内存中的内容写入硬盘
                }
            }
